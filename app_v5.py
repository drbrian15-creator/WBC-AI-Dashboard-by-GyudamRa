...
# =======================================================================
# 🧠 [v5.0 최종 정밀 개혁] 89MB 대용량 뇌 파일 GitHub LFS 필터링 우회 로드
# =======================================================================
@st.cache_resource
def load_wbc_model_bypass():
    model_path = 'wbc_model_v5.pkl'
    
    # 만약 가상 서버 환경에 진짜 파일이 없거나 껍데기(용량이 1MB 미만)만 존재한다면 다이렉트 스트리밍 다운로드
    if not os.path.exists(model_path) or os.path.getsize(model_path) < 1000000:
        with st.spinner("🔮 GitHub LFS 우회 프로토콜 가동: 대용량 AI 앙상블 뇌 엔진을 서버에 이식 중입니다... (최초 1회 약 5초 소요)"):
            raw_url = "https://raw.githubusercontent.com/drbrian15-creator/wbc-ai-dashboard-by-gyudamra/main/wbc_model_v5.pkl"
            urllib.request.urlretrieve(raw_url, model_path)
            
    return joblib.load(model_path)

try:
    model = load_wbc_model_bypass()
